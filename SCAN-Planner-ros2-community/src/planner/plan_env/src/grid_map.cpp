...
void GridMap::initMap(rclcpp::Node *node)
{
  ...
  load_parameter(node_, "grid_map.enable_cloud_subscription", mp_.enable_cloud_subscription_, true);
  ...
  else if (mp_.sensor_type_ == "lidar")
  {
    ...
    if (mp_.enable_cloud_subscription_)
    {
    ...
    else
    {
      RCLCPP_WARN(node_->get_logger(),
                  "[GridMap] LOCAL OBSTACLE CLOUD SUBSCRIPTION IS DISABLED; "
                  "the local occupancy grid will remain empty");
    }
  }
